...
from commons import output_dir, log_file
from config.env import BASE_URL, Log
...
def generate_schema_columns(header):
    ...
    for elem in header:
        if elem in ["channel_callback_id"]:
            column_type = "long"
        elif elem == 'created_at' or elem == 'agent_deleted_at' or elem == 'user_deleted_at':
...
def update_schema(name, updated_schema_column, token):
    ...
    response = requests.put(f'{BASE_URL}/datasets/{Log}', headers=header, json=payload)
...
def upload_csv_log():
    ...
    csv_path = os.path.join(current_directory, output_dir, log_file)
    ...
    if os.path.isfile(csv_path):
        ...
        try:
            ...
            response = requests.put(f'{BASE_URL}/datasets/{Log}/data', headers=header, data=csv_data)

            # Check the API status
            if response.status_code == 204:
                logger.info("Data imported successfully")
            else:
                error_message = f'Error {response.status_code} for file {log_file}: {response.text}'
                logger.error(f'Error {response.status_code} for file {log_file}: {response.text}')
                # send_email_error(error_message)
        ...
        error_message = f"CSV file {log_file} not found in {output_dir} folder."
        logger.info(f"CSV file {log_file} not found in {output_dir} folder.")
        # send_email_error(error_message)
