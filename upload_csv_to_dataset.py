...
from commons import output_dir, account_file
...
def generate_schema_columns(header):
    ...
    for elem in header:
        if elem in ["id",]:
            column_type = "long"
        elif elem in ["name", "displayName", "type", "valid", "dataProviderType",
                      "credentialsType","userId",'createdBy','modifiedBy']:
            column_type = "string"
        elif elem == 'createdAt'or elem == 'modifiedAt':
...
def upload_csv():
    ...
    csv_path = os.path.join(current_directory, output_dir, account_file)
    ...
    if os.path.isfile(csv_path):
        ...
        try:
            ...
            if response.status_code == 204:
                ...
                error_message = f'Error {response.status_code} for file {account_file}: {response.text}'
                logger.error(f'Error {response.status_code} for file {account_file}: {response.text}')
                send_email_error(error_message)
        ...
        error_message = f"CSV file {account_file} not found in {output_dir} folder."
        logger.info(f"CSV file {account_file} not found in {output_dir} folder.")
        send_email_error(error_message)
